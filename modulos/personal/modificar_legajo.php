...
function form_modificar($nuevo = false,$id = "") {
  ...
  $id_afjp=$_POST['leg_afjp'];
	...
	//Vincula Expedientes   
	$modo_exd=$parametros["modo_exd"];
	$id_legajo_x_exd=$parametros["id_legajo_x_exd"];

	if ($modo_exd=="desvincula_exd"){
		$query="delete from expedientes.legajo_x_exd where id_legajo_x_exd='$id_legajo_x_exd'";
		sql($query, "Error al desvincula exd") or fin_pagina();
	}

	if($id_legajo!=""){?>
	
		<tr>
			<td style=\"border:$bgcolor3;cursor:hand;\" colspan=2 align=center id='mo' onclick="muestra_tabla(document.all.tabla_exd,8);">
				<img id="imagen_8" src="../../imagenes/drop2.gif" border=0 title="Mostrar columnas" align="left" style="cursor:hand;">
				<font size=+1>Expedientes</font>
			</td>
		</tr>
		
		<tr>
			<td colspan="2" align=center>
				<?$sel_sum="select id_legajo_x_exd,expediente.id_expediente, extracto
								from expedientes.legajo_x_exd 
								inner join expedientes.expediente ON legajo_x_exd.id_expediente = expediente.id_expediente
								where id_legajo=$id_legajo";
				$sumario=sql($sel_sum,"No se pudo recuperar los exd") or fin_pagina();
				$can_sum=$sumario->RecordCount();?>
				<table id="tabla_exd" style="display:none;" align="center" width="80%">
				<tr>
					<td>
	    				<table align="center" width="100%" border="1">
	  						<?if($can_sum==0){?>
		    					<tr>
		      						<td align="center"><font color="Red" size="2"><b>No Existen Expedientes Relacionados</b></font></td>
		    					</tr>
	    					<?}
	    					else {?>
							    <tr id="mo">
							  		<td width="15%"><b>Expediente</b></td>
							        <td width="75%"><b>Extracto</b></td>
							        <td width="10%"><b>Desvincular</b></td>
							  	</tr>
	   							<?$i=1;
	    							while($i<=$can_sum){
								        $link = encode_link("../expedientes/exd_nuevo.php",array("id_expediente"=>$sumario->fields['id_expediente'],"pagina_viene"=>"exd_listado_tot.php"));
								        $onclick_elegir="window.open('$link','','top=50, left=170, width=1024, height=768, scrollbars=1, status=1,directories=0');";?>
	      								<tr <?=atrib_tr()?>>
									      <td onclick="<?=$onclick_elegir?>"><b><?=$sumario->fields['id_expediente']?></b></td>
									      <td onclick="<?=$onclick_elegir?>"><b><?=$sumario->fields['extracto']?></b></td>
									      
									      <?$lnk=encode_link("$_SERVER[PHP_SELF]",Array("id_legajo_x_exd"=>$sumario->fields['id_legajo_x_exd'],"modo_exd"=>"desvincula_exd","cmd"=>"modificar","id_legajo"=>$id_legajo,"pagina"=>"listado_legajo"));
		      							  $onclick_eliminar="if (confirm('Esta Seguro que Desea Desvincular el Expediente ".$sumario->fields['id_expediente']."?')) location.href='$lnk'
	            												else return false;	";?>
		        			...
		        			$link = encode_link("../expedientes/vincula_exd.php",array("id_legajo"=>$id_legajo,"pagina_viene"=>"modificar_legajo.php"));
		        			$link1="window.open('$link','','top=50, left=170, width=1024, height=768, scrollbars=1, status=1,directories=0');";?>
							<tr id="mo">
			      				<td align="center" colspan="3"><input type="button" name=nuevo value='Vincular Expediente' onclick="<?=$link1?>"></td>
			    			</tr>   
	 					</table>
	 				</td>
				</tr>
...
//Vincula Contratos   
	$modo_con=$parametros["modo_con"];
	$id_contratacion=$parametros["id_contratacion"];

	if ($modo_con=="desvincula_con"){
		$query="delete from expedientes.contratacion where id_contratacion='$id_contratacion'";
		sql($query, "Error al desvincula exd") or fin_pagina();
	}
	...
	if($id_legajo!=""){?>
		...
		<tr>
			<td style=\"border:$bgcolor3;cursor:hand;\" colspan=2 align=center id='mo' onclick="muestra_tabla(document.all.tabla_con,9);">
				<img id="imagen_9" src="../../imagenes/drop2.gif" border=0 title="Mostrar columnas" align="left" style="cursor:hand;">
				<font size=+1>Contrataciones</font>
			</td>
		</tr>
		...
		<tr>
			<td colspan="2" align=center>
				<?$sel_sum="SELECT
							expedientes.contratacion.id_contratacion,
							expedientes.contratacion.id_legajo,
							expedientes.contratacion.fecha_inicio,
							expedientes.contratacion.fecha_fin,
							expedientes.contratacion.observaciones,
							expedientes.contratacion.honorario,
							expedientes.area_con.desc_area_con,
							expedientes.modalidad_con.desc_modalidad_con,
							expedientes.fuente_con.desc_fuente_con,
							expedientes.funcion_con.desc_funcion_con,
							expedientes.metodo_con.desc_metodo_con,
							expedientes.tdr_con.desc_tdr_con
							FROM
							expedientes.contratacion
							INNER JOIN expedientes.area_con ON expedientes.contratacion.id_area_con = expedientes.area_con.id_area_con
							INNER JOIN expedientes.fuente_con ON expedientes.contratacion.id_fuente_con = expedientes.fuente_con.id_fuente_con
							INNER JOIN expedientes.funcion_con ON expedientes.contratacion.id_funcion_con = expedientes.funcion_con.id_funcion_con
							INNER JOIN expedientes.metodo_con ON expedientes.contratacion.id_metodo_con = expedientes.metodo_con.id_metodo_con
							INNER JOIN expedientes.modalidad_con ON expedientes.contratacion.id_modalidad_con = expedientes.modalidad_con.id_modalidad_con
							INNER JOIN expedientes.tdr_con ON expedientes.contratacion.id_tdr_con = expedientes.tdr_con.id_tdr_con
 							where id_legajo=$id_legajo";
				$sumario=sql($sel_sum,"No se pudo recuperar los exd") or fin_pagina();
				$can_sum=$sumario->RecordCount();?>
				<table id="tabla_con" style="display:none;" align="center" width="80%">
				<tr>
					<td>
	    				<table align="center" width="100%" border="1">
	  						<?if($can_sum==0){?>
		    					<tr>
		      						<td align="center"><font color="Red" size="2"><b>No Existen Contrataciones Relacionados</b></font></td>
		    					</tr>
	    					<?}
	    					else {?>
							    <tr id="mo">
							  		<td width="15%"><b>Fecha Inicio</b></td>
							        <td width="75%"><b>Fecha Fin</b></td>
							        <td width="75%"><b>Modalidad</b></td>
							        <td width="75%"><b>Funcion</b></td>
							        <td width="10%"><b>Desvincular</b></td>
							  	</tr>
	   							<?$i=1;
	    							while($i<=$can_sum){
								        $link = encode_link("../expedientes/con_nuevo.php",array("id_contratacion"=>$sumario->fields['id_contratacion'],"pagina_viene"=>"modificar_legajo.php","id_legajo"=>$id_legajo));
								        $onclick_elegir="window.open('$link','','top=50, left=170, width=1024, height=768, scrollbars=1, status=1,directories=0');";?>
	      								<tr <?=atrib_tr()?>>
									      <td onclick="<?=$onclick_elegir?>"><b><?=fecha($sumario->fields['fecha_inicio'])?></b></td>
									      <td onclick="<?=$onclick_elegir?>"><b><?=fecha($sumario->fields['fecha_fin'])?></b></td>
									      <td onclick="<?=$onclick_elegir?>"><b><?=$sumario->fields['desc_modalidad_con']?></b></td>
									      <td onclick="<?=$onclick_elegir?>"><b><?=$sumario->fields['desc_funcion_con']?></b></td>
									      
									      <?$lnk=encode_link("$_SERVER[PHP_SELF]",Array("id_contratacion"=>$sumario->fields['id_contratacion'],"modo_con"=>"desvincula_con","cmd"=>"modificar","id_legajo"=>$id_legajo,"pagina"=>"listado_legajo"));
		      							  $onclick_eliminar="if (confirm('Esta Seguro que Desea Eliminar el Registro ".$sumario->fields['id_contratacion']."?')) location.href='$lnk'
	            												else return false;	";?>
		      							  <td onclick="<?=$onclick_eliminar?>" align="center"><img src='../../imagenes/sin_desc.gif' style='cursor:hand;'></td>
									    </tr>
	    								<?$sumario->MoveNext();
		        			...
		        			$link = encode_link("../expedientes/con_nuevo.php",array("id_legajo"=>$id_legajo,"pagina_viene"=>"modificar_legajo.php"));
		        			$link1="window.open('$link','','top=50, left=170, width=1024, height=768, scrollbars=1, status=1,directories=0');";?>
							<tr id="mo">
			      				<td align="center" colspan="5"><input type="button" name=nuevo value='Vincular Contratacion' onclick="<?=$link1?>"></td>
			    			</tr>   
	 					</table>
	 				</td>
				</tr>
...
?>
      ...
      <tr>
        ...
        <tr><td colspan="2"><hr><b>Area:</b></td></tr>
	  ...
	  <td valign="top"><table>	         
	         ...
	         <tr><td colspan="2"><hr><b>Obra Social:</b></td></tr>
	         <tr><td align="left">
                 <INPUT TYPE='RADIO' name='leg_jub' value='0' <? if ($tipo_jub==0) echo 'checked'?>>&nbsp;<b>Obra Social</b>
            </td>
             ...
             <tr><td align="left" colspan="2">
                 <INPUT TYPE='RADIO' name='leg_jub' value='1' <?if ($tipo_jub==1) echo 'checked'?>>&nbsp;<b>Sin Obra Social</b>
             </td></tr>
             <tr>
